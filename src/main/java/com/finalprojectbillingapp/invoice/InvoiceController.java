...
//            return "redirect:/new-invoice/";
    ...
    @GetMapping("/createNewInvoice/currency")
    public String displayCurrencyPage(Model model) {
        model.addAttribute("currencies", Currency.values());
        return "currencyForInvoice";
    }

    @PostMapping("/createNewInvoice/currency")
    public String selectCurrency(@RequestParam("currency") Currency selectedCurrency) {
...
//            selectedCustomer = this.customerService.getCustomerById(selectedCustomer.getId());
    ...
    public String convertToPdfInvoiceOverview( Model model, HttpSession session) {
        try {
            UUID invoiceId = (UUID) session.getAttribute("newInvoiceId");
            InvoiceEntity invoice = invoiceService.getInvoiceById(invoiceId);
            List<ProductOrServiceEntity> products = this.invoiceService.getProductObjectsForInvoice(invoiceId);
            ...
            return "redirect:/?message=INVOICE_OVERVIEW_NEW_FAILED&error="
                    + exception.getMessage();
        }
    }
